...
export const TxHashDiv : FC <props> = ({
    ...
    const div = () =>{
        ...
        if (txHash instanceof Error){
            ...
            return <CommonMessageDiv style={{width:"99.8%"}}
            className="text-left w-full p-2 bg-red-600 text-gray-100 rounded-xl mb-2"
            transitionDuration={0.32}>{txHash.message}</CommonMessageDiv>
        }
        else {
    
            return <CommonMessageDiv style={{width:"99.8%"}}
            className="text-left w-full p-2 bg-cyan-800 text-gray-100 rounded-xl mb-2"
